...
SEHIRLER = ["Adana", "Adiyaman", "Afyonkarahisar", "Agri", "Amasya", "Ankara", "Antalya", "Artvin", "Aydin", "Balikesir", "Bilecik", "Bingol", "Bitlis", "Bolu", "Burdur", "Bursa", "Canakkale", "Cankiri", "Corum", "Denizli", "Diyarbakir", "Edirne", "Elazig", "Erzincan", "Erzurum", "Eskisehir", "Gaziantep", "Giresun", "Gumushane", "Hakkari", "Hatay", "Isparta", "Mersin", "Istanbul", "Izmir", "Kars", "Kastamonu", "Kayseri", "Kirklareli", "Kirsehir", "Kocaeli", "Konya", "Kutahya", "Malatya", "Manisa", "Kahramanmaras", "Mardin", "Mugla", "Mus", "Nevsehir", "Nigde", "Ordu", "Rize", "Sakarya", "Samsun", "Siirt", "Sinop", "Sivas", "Tekirdag", "Tokat", "Trabzon", "Tunceli", "Sanliurfa", "Usak", "Van", "Yozgat", "Zonguldak", "Aksaray", "Bayburt", "Karaman", "Kirikkale", "Batman", "Sirnak", "Bartin", "Ardahan", "Igdir", "Yalova", "Karabuk", "Kilis", "Osmaniye", "Duzce"]
...
# Türkçe karakterleri PDF için düzeltme
def tr_pdf(text):
    replacements = {
...
# --- PDF TEKLİF OLUŞTURUCU (Sıfırdan Oluşturur) ---
def pdf_teklif_olustur(firma_adi, iskonto_pompa, iskonto_istasyon, odeme_sekli, yetkili):
    try:
        ...
        pdf.set_font("Arial", size=12)
        # Türkçe karakterleri düzelt (fpdf standart fontu Türkçe desteklemez)
        text = tr_pdf(f"""Sayin {yetkili} ({firma_adi}),

Ozkaraaslan Filo ve Petrol Ofisi guvencesiyle, firmanizin akaryakit ihtiyaclarini karsilamak adina hazirladigimiz ozel teklifimiz asagidaki gibidir.

Otomatik Filo Yonetim Sistemi ile araclariniz istasyonlarimizda ucret odemeden yakit alabilir, tum tuketimlerinizi tek faturada yonetebilirsiniz.""")
        
        pdf.multi_cell(0, 7, text)
        pdf.ln(10)
        
        # Tablo Başlığı
        pdf.set_fill_color(227, 6, 19) # Kırmızı
        pdf.set_text_color(255, 255, 255)
        pdf.set_font("Arial", 'B', 12)
        pdf.cell(95, 10, "Hizmet / Detay", 1, 0, 'C', True)
        pdf.cell(95, 10, "Kosullar", 1, 1, 'C', True)
        
        # Tablo İçeriği
        pdf.set_text_color(0, 0, 0)
        pdf.set_font("Arial", size=12)
        
        pdf.cell(95, 10, "Pompa Iskontosu", 1, 0)
        pdf.cell(95, 10, f"% {iskonto_pompa}", 1, 1, 'C')
        
        pdf.cell(95, 10, "Anlasmali Istasyon Iskontosu", 1, 0)
        pdf.cell(95, 10, f"% {iskonto_istasyon}", 1, 1, 'C')
        
        pdf.cell(95, 10, "Odeme Sekli", 1, 0)
        pdf.cell(95, 10, tr_pdf(odeme_sekli), 1, 1, 'C')
...
# --- YENİ TAB: TEKLİF & HESAP ---
    ...
    tab_hesap, tab_pdf = st.tabs(["💰 Tasarruf Hesapla", "📑 Teklif Oluştur"])
    ...
    with tab_pdf:
        ...
        with st.form("pdf_form"):
            ...
            generate_btn = st.form_submit_button("📄 Oluştur (Word & PDF)")
        
        # İndirme Butonları (Form Dışında)
        if generate_btn:
            if p_firma:
                ...
                try:
                    ...
                    if word_bytes:
                        col_dl1.download_button("📥 WORD İNDİR", word_bytes, f"{p_firma}_Teklif.docx", "application/vnd.openxmlformats-officedocument.wordprocessingml.document", type="primary")
                    else:
                        col_dl1.error("Word Şablonu Yok!")
                except Exception as e: col_dl1.error(f"Word Hatası: {e}")
                
                # 2. PDF OLUŞTUR
                try:
                    pdf_bytes = pdf_teklif_olustur(p_firma, p_iskonto_pompa, p_iskonto_istasyon, p_odeme, p_yetkili)
                    if pdf_bytes:
                        col_dl2.download_button("📥 PDF İNDİR", pdf_bytes, f"{p_firma}_Teklif.pdf", "application/pdf", type="secondary")
                    else:
                        col_dl2.error("PDF Oluşturulamadı")
                except Exception as e: col_dl2.error(f"PDF Hatası: {e}")
