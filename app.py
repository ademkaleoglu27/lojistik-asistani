...
# --- 1. SAYFA VE TASARIM AYARLARI (PREMIUM UI) ---
st.set_page_config(
    page_title="Lojistik Pro",
    page_icon="🚛",
    layout="wide",
    initial_sidebar_state="collapsed" # Telefondan girince menü kapalı başlasın, ekran ferah olsun
)

# --- 2. ÖZEL CSS (MAKJAJ KUTUSU) ---
def local_css():
    st.markdown("""
    <style>
        /* Google Fontunu İçe Aktar (Modern Görünüm) */
        @import url('https://fonts.googleapis.com/css2?family=Inter:wght@400;600;700&display=swap');

        html, body, [class*="css"] {
            font-family: 'Inter', sans-serif;
            background-color: #f8f9fa; /* Çok açık gri (Göz yormayan arka plan) */
        }
        ...
        /* Buton Tasarımları (Yuvarlak ve Gölgeli) */
        .stButton>button {
            border-radius: 12px;
            font-weight: 600;
            border: none;
            ...
            height: 50px; /* Daha kolay tıklanabilir */
        }
        ...
        /* Kart Görünümü (Metrikler ve Kutular için) */
        ...
        /* Gereksiz Streamlit Menüsünü Gizle */
        #MainMenu {visibility: hidden;}
        footer {visibility: hidden;}
        header {visibility: hidden;} /* Üstteki renkli çubuğu da gizle */
        
    </style>
    """, unsafe_allow_html=True)

local_css() # CSS'i çalıştır
...
@st.cache_data(ttl=60) # 60 saniye önbellek (Hız için)
def veri_tabanini_yukle():
    try:
        ...
        if not data:
            basliklar = ["Firma", "Telefon", "Web", "Email", "Adres", "Durum", "Notlar", "Sozlesme_Tarihi", "Hatirlatici_Tarih"]
...
def veriyi_kaydet(df):
    try:
        ...
        st.cache_data.clear() # Kayıttan sonra önbelleği temizle ki yeni veri görünsün
...
with tab_home:
    ...
    if "Hatirlatici_Tarih" in df.columns:
        ...
        if not isler.empty:
            ...
            st.success("✅ Bugün için acil bir hatırlatma yok. Sahaya odaklanabilirsiniz.")

# --- TAB 2: GELİŞMİŞ ARAMA ---
with tab_search:
    with st.container(): # Gruplama kutusu
    ...
    if 'sonuclar' in st.session_state:
        ...
        if st.button("💾 SEÇİLENLERİ KAYDET", type="primary", use_container_width=True):
            ...
            if not secilenler.empty:
                with st.spinner("Mail adresleri taranıyor ve kaydediliyor..."):
                    # Mail bulma
                    ...
                    veriyi_kaydet(yeni)
                st.success(f"✅ {len(secilenler)} firma başarıyla eklendi!")
                time.sleep(1)
            else:
...
with tab_crm:
    ...
    if not df_crm.empty:
        ...
        df_crm["Mail_At"] = df_crm.apply(lambda x: mail_linki_yap(x["Email"], x["Firma"]), axis=1)
        ...
        st.info("Listeniz boş. 'Firma Bul' sekmesinden ekleme yapın.")
