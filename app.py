...
st.set_page_config(
    ...
    page_icon="⛽", 
    layout="wide",
    initial_sidebar_state="collapsed" # Menü kapalı başlar, ok ile açarsın
)
...
def local_css():
    ...
    <style>
        ...
        /* GİZLEME AYARLARI (OK İŞARETİ İÇİN HEADER GÖRÜNÜR BIRAKILDI) */
        #MainMenu {visibility: hidden;} 
        footer {visibility: hidden;} 
        /* header {visibility: hidden;}  <-- BU SATIR SİLİNDİ Kİ OK GÖRÜNSÜN */
...
# --- YAN MENÜ (SOL TARAFA ALINDI) ---
with st.sidebar:
    st.image(LOGO_URL, width=150)
    st.markdown("""
    <div class="sidebar-header">
        <h3>ÖZKARAASLAN FİLO</h3>
        <p>Saha Yönetim Paneli</p>
    </div>
    """, unsafe_allow_html=True)
